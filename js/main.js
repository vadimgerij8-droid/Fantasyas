...
// ================= Система навігації для Smart TV пульта =================
(function setupTVNavigation() {
  // Отримує всі видимі елементи з атрибутом data-tv
  function getTVFocusableElements() {
    const all = document.querySelectorAll('[data-tv]');
    return Array.from(all).filter(el => {
      // Перевіряємо, чи елемент видимий (не display: none, не прихований через батька)
      const style = window.getComputedStyle(el);
      return style.display !== 'none' && style.visibility !== 'hidden' && el.offsetParent !== null;
    });
  }

  // Знаходить найближчий елемент у заданому напрямку
  function findNearestInDirection(elements, currentRect, direction) {
    let best = null;
    let bestDistance = Infinity;

    elements.forEach(el => {
      if (el === document.activeElement) return; // пропускаємо поточний
      const rect = el.getBoundingClientRect();

      // Перевіряємо, чи елемент знаходиться у відповідному напрямку
      ...
      // Обчислюємо відстань (центр до центру)
  ...
  function onKeyDown(e) {
    const key = e.key;
    const handledKeys = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Enter', 'Backspace'];

    if (!handledKeys.includes(key)) return;

    e.preventDefault(); // Запобігаємо стандартній поведінці (скрол, тощо)
    ...
    // Для стрілок: навігація між [data-tv] елементами
    ...
    if (current && elements.includes(current)) {
      ...
      nextElement = findNearestInDirection(elements, currentRect, key);
    }

    // Якщо не знайшли або немає поточного фокуса, фокусуємо перший елемент
    if (!nextElement) {
      // Для випадку, коли поточний фокус не серед data-tv, пробуємо знайти найближчий до центру екрану?
      // Просто беремо перший
  ...
  // Опціонально: додаємо клас для стилізації фокусу (можна налаштувати в CSS)
  // Наприклад, можна додати :focus-visible або окремий клас .tv-focused
  // Але залишимо це на розсуд розробника
})();
