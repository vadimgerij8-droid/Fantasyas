...
import { 
  currentUser, currentUserData, currentChatId, currentChatPartner, currentChatPartnerName,
  currentChatPartnerUserId, currentChatPartnerAvatar, replyContext, setCurrentChat, clearChatState,
  setReplyContext, clearReplyContext, unsubscribeMessages, unsubscribeTyping, unsubscribeChatPresence,
...
export async function handleMessageContextAction(action) {
  ...
  switch (action) {
    case 'reply':
      setReplyContext(selectedMessageId, msgData.text, msgData.from === currentUser.uid ? 'Ви' : currentChatPartnerName);
