...
import {
  ...
  getDocs, serverTimestamp, arrayUnion, arrayRemove, increment, writeBatch, onSnapshot, where
...
export function renderPosts(docs, containerId = 'feed') {
  const feed = typeof containerId === 'string' ? document.getElementById(containerId) : containerId;
  if (!feed) return;
  ...
  docs.forEach(docSnap => {
    ...
    const unsubscribe = onSnapshot(postRef, (snap) => {
      if (snap.exists()) {
        ...
        if (likeBtn) {
          const isLiked = data.likes?.includes(state.currentUser?.uid) || false;
        ...
        if (saveBtn) {
          const isSaved = data.saves?.includes(state.currentUser?.uid) || false;
